...
public class PathUtil {
    private static Pattern ROOT_PROJECT_PATTERN = Pattern.compile("^(.*?)\\/springboot-testing-tips.*?$");
    ...
    static String getJarfile(String path) {
        ...
        File[] files = dir.listFiles(file -> file.getName().contains("weather") && file.getName().endsWith("jar"));
        if (files.length != 1) {
