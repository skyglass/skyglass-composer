...
public class PathUtil {
	private static Pattern ROOT_PROJECT_PATTERN = Pattern.compile("^(.*?)\\/doctor-booking.*?$");
	...
	static String getJarfile(String path) {
		...
		File[] files = dir.listFiles(file -> file.getName().contains("doctor") && file.getName().endsWith("jar"));
		if (files.length != 1) {
