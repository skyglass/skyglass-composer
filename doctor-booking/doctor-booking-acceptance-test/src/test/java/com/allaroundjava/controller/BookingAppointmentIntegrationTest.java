...
@SpringBootTest(webEnvironment = SpringBootTest.WebEnvironment.RANDOM_PORT, classes = { TestJpaConfig.class, AppConfig.class, IntegrationTestConfig.class })
public class BookingAppointmentIntegrationTest {
	...
	private static DoctorsApi doctorsApi;
	...
	public void havingDoctorPatientAndAppointmentSlot_thenAppointmentSlotCanBeBooked() {
		...
		DoctorDto doctorExchange = doctorsApi.createDoctor(doctorHttpEntity.getBody());
		Assert.assertTrue(doctorExchange != null);
		...
		HttpEntity<AppointmentSlotDto> appointmentSlotDtoHttpEntity = getAppointmentSlotDtoHttpEntity(doctorExchange.getEntityId(), appointmentStart, appointmentEnd);
